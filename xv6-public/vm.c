...
int loaduvm(pde_t *pgdir, char *addr, struct inode *ip, uint offset, uint sz, uint elf_flags)
{
  uint i, pa, n;
  pte_t *pte;
  ...
  {                                                 // Iterate over every page that needs to be loaded
...
int wunmap(uint addr)
{
  ...
  if ((m == 0) | (m->inuse != 1))
  {
    cprintf("No such mapping exists.\n");
    return -1;
  }
